...
db.drop_all()
db.create_all()
quotesForQuoteModel = [QuoteModel('Народная мудрость', 'Нет пламя без огня'),
...
def edit_quote(quote_id):
    ...
    if "author" in new_quote and "text" in new_quote:
...
def quoteModel_parse(quoteModel):
    quote = {"id": quoteModel.id, "author": quoteModel.author, "text": quoteModel.text}
    return quote
